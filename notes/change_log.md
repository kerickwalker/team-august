...
## 2026-04-07: Pure Pursuit path-following controller

### Goal
Enable the robot to follow a pre-generated trajectory (CSV file) using the Pure Pursuit
algorithm, with the Kalman-filtered pose as the feedback source.

### New files

#### `mqtt_python/spursuit.py`
Pure Pursuit math module. No MQTT, no robot imports. Loads a CSV with columns
`x, y, heading, cumulative_dist` (arc-length spacing ~0.05 m). Implements:
- `compute_command(x, y, heading, speed)` → `(linvel, turnrate)` using bounded nearest-
  point search, lookahead projection, robot-frame transform, and curvature formula.
- `at_end()`, `reset()`, `is_loaded()` for state management.
- Singleton `pursuit = SPurePursuit()`.
- Tunable constants: `LOOKAHEAD_DIST = 0.3 m`, `MAX_LINVEL = 0.3 m/s`,
  `MAX_TURNRATE = 4.0 rad/s`, `SEARCH_WINDOW = 30`.

#### `mqtt_python/spath_follow.py`
MQTT integration layer. Has its own MQTT client subscribed to `robobot/kalman/state`
(separate from `uservice`; the Kalman topic is outside `robobot/drive/#`). Handles both
JSON schemas the Kalman node may emit. API mirrors `sedge.lineControl()`:
- `pathControl(velocity)` — arm (velocity > 0) or disarm.
- `update()` — call from mission loop at ~50 Hz; publishes `rc` command.
- `setup()` / `terminate()` integrated with `uservice`.
- `TRAJECTORY_CSV = "trajectory.csv"` hardcoded at top of file.

### Modified files

#### `mqtt_python/mqtt_client_core.py`
- Added imports: `from spath_follow import path_follow`, `from spursuit import pursuit`.
- Added `drivePathFollow()` mission function: arms controller, runs 50 Hz update loop
  until `pursuit.at_end()`, then stops and returns.
- Added state 105 (`drivePathFollow()`) in `loop()`.

#### `mqtt_python/uservice.py`
- Added `from spath_follow import path_follow` import.
- Added `path_follow.setup()` after `edge.setup()`.
- Added `path_follow.terminate()` after `edge.terminate()`.

### New documentation
- `mqtt_python/CLAUDE.md` — full project context for Claude Code sessions.
- `notes/path_following.md` — algorithm, tuning guide, edge cases, swap-out instructions.

### How to run
```bash
python3 mqtt_client_core.py --usestate 105
```
Prerequisites: `teensy_interface` running, Kalman node publishing, `trajectory.csv` present.
