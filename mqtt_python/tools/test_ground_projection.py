...
Usage:
    python3 test_ground_projection.py --host 10.197.216.254

Controls:
    +  / -       tilt  +/- 0.5 deg
    W  / S       height +/- 1 cm
    A  / D       roll  +/- 0.5 deg
    R            reset to camera_extrinsics.py values
    P            print current extrinsics
    Q / ESC      quit

    Left click   project that pixel -> prints X,Y to terminal
"""
...
def draw_overlay(frame, grid_results):
    ...
    hud = [
        ...
        f"roll  : {ground.camera_roll:.1f} deg   ( A/D )",
...
def main():
    parser = argparse.ArgumentParser(description="Live ground projection test")
