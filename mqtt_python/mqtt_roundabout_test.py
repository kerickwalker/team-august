...
def loop():
  ...
  edge.lineControl(0)   # ensure motors are off at start

  if not service.is_quiet():
    print("% Roundabout test: phase 1 = line follow, phase 2 = arc")
  ...
  # Short pause between phases so the robot is fully stopped
  t.sleep(0.1)

  # Phase 2: drive the roundabout circle
  driveRoundabout()
  ...
  if not service.is_quiet():
    print("% Roundabout test: complete")
