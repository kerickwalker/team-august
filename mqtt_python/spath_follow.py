...
class SPathFollow:
    ...
    def pathControl(self, velocity: float):
        ...
        if velocity > 0.001:
            ...
            # Step 1 — Translate: align trajectory start with current robot position.
            # Trajectory convention: col 0 = right (+x), col 1 = forward (+y).
            # Kalman convention:     x = forward,         y = left.
            traj[:, 0] += -self._y - traj[0, 0]      # right:   -Kalman_y(left) = traj_x
            traj[:, 1] +=  self._x - traj[0, 1]      # forward:  Kalman_x       = traj_y

            # Step 2 — Rotate: align trajectory's initial direction with the current
            # Kalman heading (= 0 after reset, i.e. the robot's current facing direction).
            dx0 =  traj[1, 1] - traj[0, 1]           # traj forward (col 1) → Kalman x
            dy0 = -(traj[1, 0] - traj[0, 0])         # -traj right  (col 0) → Kalman y (left)
            theta0   = math.atan2(dy0, dx0)
            rotation = self._heading - theta0          # self._heading ≈ 0 after kalman.reset()
            if abs(rotation) > 1e-4:
                cx, cy   = traj[0, 0], traj[0, 1]
                dx_k =  (traj[:, 1] - cy)             # Kalman x from traj forward (col 1)
                dy_k = -(traj[:, 0] - cx)             # Kalman y from -traj right  (col 0)
                cos_r, sin_r = math.cos(rotation), math.sin(rotation)
                new_dx_k = cos_r * dx_k - sin_r * dy_k
                new_dy_k = sin_r * dx_k + cos_r * dy_k
                traj[:, 1] = cy + new_dx_k            # traj forward (col 1) = Kalman x
                traj[:, 0] = cx - new_dy_k            # traj right  (col 0) = -Kalman y
                traj[:, 2] += rotation
                print(f"% SPathFollow: trajectory rotated {math.degrees(rotation):.1f}° "
                      f"(traj initial heading was {math.degrees(theta0):.1f}° in Kalman frame)")
