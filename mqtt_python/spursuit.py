...
LOOKAHEAD_DIST = 0.75   # metres — distance ahead on trajectory to aim for
MAX_LINVEL     = 0.2   # m/s   — constant forward speed during tracking
MAX_TURNRATE   = 1.0   # rad/s — saturation limit for angular velocity output
SEARCH_WINDOW  = 50    # max points to scan forward when finding nearest point
...
class SPurePursuit:
    ...
    def compute_command(self, x: float, y: float, heading: float, speed: float, linvel: float = None):
        ...
        # Trajectory: col 0 = right (+x), col 1 = forward (+y).
        # Kalman frame: x = forward, y = left.
        # Convert: Kalman_x = traj_y (col 1), Kalman_y = -traj_x (left = -right).
        end_search = min(self._nearest_idx + SEARCH_WINDOW, N)
        seg_x =  traj[self._nearest_idx:end_search, 1] - x   # traj forward → Kalman x
        seg_y = -traj[self._nearest_idx:end_search, 0] - y   # -traj right  → Kalman y (left)
        ...
        while lookahead_idx < N - 1 and traj[lookahead_idx, 3] < target_cum_dist:
            lookahead_idx += 1
        x_l =  traj[lookahead_idx, 1]   # traj forward (+y) → Kalman x (forward)
        y_l = -traj[lookahead_idx, 0]   # -traj right (+x)  → Kalman y (left)
