...
#
#  [END]      Nudge clear → turn right 45° → drive 1.1 m → turn right 90°
#             → drive 0.15 m → drop flag → done.
#
# ── How to tune ──────────────────────────────────────────────────────────────
#
#  All tunable values live in the CONFIGURATION section below, grouped by
#  mission phase (LINE, CROSSINGS, CROSSING1, ROUNDABOUT_*, LINE_END, END).
...
LINE = SimpleNamespace(
    speed        = 0.20,   # m/s — normal line-following speed
...
# ── Crossing 1 — turn left, then resume line following ───────────────────────
CROSSING1 = SimpleNamespace(
    turn_deg  = 45.0,    # degrees to turn
...
ROUNDABOUT_ENTRY = SimpleNamespace(
    drive_dist_m = 0.20,    # m — drive forward after line ends before turning
    drive_speed  = 0.15,    # m/s
    turn_deg     = 45.0,    # degrees to turn to face the roundabout circle
    turn_dir     = "left",  # "left" or "right"
)
...
ROUNDABOUT_ARC = SimpleNamespace(
    diameter_m    = 0.8,     # m — physical diameter of the circle
...
ROUNDABOUT_EXIT = SimpleNamespace(
    turn_deg     = 90.0,    # degrees to turn after the arc
    turn_dir     = "left",  # "left" or "right"
    seek_speed   = 0.15,    # m/s while searching for the line
    seek_timeout = 10.0,    # s — give up if line not found within this time
)
...
# ── End sequence — runs after the final crossing ──────────────────────────────
END = SimpleNamespace(
    clear_dist_m  = 0.10,    # m — nudge forward to clear the crossing markings
    turn1_deg     = 45.0,    # ° — first turn angle (right)
    drive1_dist_m = 1.10,    # m — main drive away from the crossing
    turn2_deg     = 90.0,    # ° — second turn angle (right)
    drive2_dist_m = 0.15,    # m — short final drive
    drive_speed   = 0.20,    # m/s — speed for end-sequence drives
    servo_up      = -800,    # servo PWM: arm raised
    servo_down    =  100,    # servo PWM: arm lowered (flag drop)
    flag_hold_s   = 3.0,     # s — how long to hold the flag down
)
...
#   "servo"     — move servo: pos (PWM value), hold (s, optional — waits before continuing)
################################################################
...
# Steps executed after the final crossing → end of mission
END_SEQUENCE = [
    ("servo", {"pos": END.servo_up}),                                      # ensure arm is raised
    ("drive", {"dist": END.clear_dist_m,   "speed": END.drive_speed}),    # clear the crossing
    ("turn",  {"deg":  END.turn1_deg,       "dir":   "right"}),
    ("drive", {"dist": END.drive1_dist_m,  "speed": END.drive_speed}),    # drive away
    ("turn",  {"deg":  END.turn2_deg,       "dir":   "right"}),
    ("drive", {"dist": END.drive2_dist_m,  "speed": END.drive_speed}),
    ("servo", {"pos": END.servo_down, "hold": END.flag_hold_s}),           # drop flag
    ("servo", {"pos": END.servo_up}),                                      # raise arm
]
...
def driveDistance(dist, speed=0.2):
    ...
    Waits until odometry distance (pose.tripB) reaches the target."""
    pose.tripBreset()
    state = 0
    if not service.is_quiet():
        print(f"% driveDistance: {dist:.2f} m at {speed:.2f} m/s")
    service.send("robobot/cmd/T0", "leds 16 0 100 0")
    while not service.stop:
        if state == 0:
            ...
            if pose.tripB >= dist or pose.tripBtimePassed() > 20:
                service.send("robobot/cmd/ti", "rc 0.0 0.0")
                state = 2
        elif state == 2:
...
def run_sequence(steps):
    ...
    for action, params in steps:
        ...
        if action == "drive":
            ...
            service.send("robobot/cmd/T0", f"servo 1 {params['pos']} 100")
            if "hold" in params:
                t.sleep(params["hold"])
        else:
            if not service.is_quiet():
    ...
    while not service.stop:
        ...
        if state == 0:
            ...
            if getattr(service.args, "now", False) or ir.ir[0] < 0.2:
                service.send("robobot/cmd/T0", "servo 1 -800 100")   # raise arm
        ...
        # ── State 10: line following + crossing reactions + line-end detection ─
            ...
            if at_crossing:
                ...
                if crossing_number == 1 and not first_cross_done:
                    # ── Crossing 1: turn left, then resume ───────────────────
                    if not service.is_quiet():
        ...
        # ── State 20: end sequence ────────────────────────────────────────────
        # Runs END_SEQUENCE step-by-step, then waits for full stop.
        elif state == 20:
            run_sequence(END_SEQUENCE)
            state = 2
