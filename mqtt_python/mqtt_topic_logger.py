...
The logger keeps the raw payload and also extracts structured fields for
known payload formats to make post-processing easier.
"""
...
class MQTTTopicLogger:
    ...
    def _print_entry(self, entry) -> None:
        ...
        if topic == "robobot/kalman/state" and isinstance(parsed, dict) and "position" in parsed:
    ...
    def run(self) -> int:
        ...
        timeout_s = 5.0
        t0 = t.time()
        while not self.connected and t.time() - t0 < timeout_s:
            t.sleep(0.05)
...
def default_output_path() -> Path:
    ...
    return Path("mqtt_python") / f"mqtt_log_{stamp}.jsonl"
